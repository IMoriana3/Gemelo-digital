# Integración en el Panel de Proyectos

## a) Entrada para el array `PROJECTS`

```html
{
  name: "Gemelo Digital — Seguidor + TCU",
  status: "demo",
  objetivo: "Visor 3D en navegador de un seguidor solar de 1 eje y su unidad de control TCU, con estado en vivo (SCADA) o simulado.",
  stack: "Three.js (WebGL) · HTML/JS · glTF/GLB · SCADA REST",
  url: "https://imoriana3.github.io/dtwin-seguidor-tcu/",
  repo: "https://github.com/IMoriana3/dtwin-seguidor-tcu",
  updated: "2026-06-16",
  version: "v1.0",
  docId: "gemelo-digital-seguidor-tcu",
  download: "",
  uso: [
    "Vista partida: campo (seguidor bifila N-S) a la izquierda, CAD de la TCU a la derecha.",
    "Sliders dia/hora/nubes/viento + brujula; botones Auto/Manual, Este/Oeste, BT on/off y Tiempo (lluvia/nieve).",
    "Con SCADA (?scada=host) pinta angulo/consigna/SoC/estado reales; sin el simula sol+backtracking+bateria+viento.",
    "El CAD de la TCU carga de tcu.glb con fallback automatico a jsDelivr; conectores etiquetados y LEDs/cables con corriente.",
    "Servir por HTTP y abrir index.html (no file://); o GitHub Pages."
  ],
  pendientes: [
    { done:false, text:"Habilitar GitHub Pages para la URL fija." },
    { done:false, text:"Que la nieve/suciedad acumulada penalice corriente/SoC (ahora es solo visual)." },
    { done:false, text:"Afinar alineacion E-O de las dos filas del bifila en parcelas muy irregulares." },
    { done:true,  text:"Fallback CDN del CAD de la TCU (carga aunque el HTML se abra fuera de su origen)." },
    { done:true,  text:"Extraido a repo propio (separado de SolarGPT)." }
  ],
  historial: [
    { date:"2026-06-16", ver:"v1.0", note:"Extraido a repo propio + documentacion generada." }
  ]
}
```

## b) Bloque de documentación (`doc-src`)

Contiene el mismo README. El `data-id` coincide con el `docId`.

```html
<script type="text/plain" class="doc-src" data-id="gemelo-digital-seguidor-tcu">
...
> **Importante:** `CDN_GLB` apunta a `IMoriana3/dtwin-seguidor-tcu@main/tcu.glb`. Si
...
`https://imoriana3.github.io/dtwin-seguidor-tcu/`.
